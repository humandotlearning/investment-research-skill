...
class SourceAdapterTests(unittest.TestCase):
    ...
    def test_normalizes_yc_company_directory_snapshot(self):
        ...
        self.assertEqual(record["origins"], expected_yc_origins)
        self.assertIsNone(record["team_signal"])
        self.assertTrue(record["freshness_or_traction_signals"])
        self.assertEqual(
            {signal["kind"] for signal in record["freshness_or_traction_signals"]},
            {"freshness"},
        )
        self.assertEqual(
            {
        ...
        self.assertEqual(candidate["origins"], expected_yc_origins)
        ...
        self.assertEqual(
            ...
            ["Automates investment-team workflows."],
        )
    ...
    def test_cross_source_records_fall_back_to_normalized_name_without_two_usable_domains(self):
        product_hunt_origin = {
            "source": "product_hunt",
            "canonical_url": "https://www.producthunt.com/posts/fallback-ai",
            "source_id": "fallback-ai-launch",
            "publication_or_batch_date": "2026-08-21T12:00:00Z",
        }
        yc_origin = {
            "source": "yc",
            "canonical_url": "https://www.ycombinator.com/companies/fallback-ai",
            "source_id": "84",
        ...
        product_hunt = {
            "name": "Fallback.AI",
            "slug": "fallback-ai",
            "website": "https://fallback.example/launch",
            "one_line_description": "Automates fallback research workflows.",
            "origins": [product_hunt_origin],
            "team_signal": None,
            "freshness_or_traction_signals": [{
                "kind": "freshness",
                "source_url": product_hunt_origin["canonical_url"],
            }],
            "thesis_fit_reasons": ["Automates investment-team workflows."],
        }
        yc = {
            "name": "Fallback AI",
            "slug": "fallback-ai",
            ...
            "freshness_or_traction_signals": [{
                "kind": "freshness",
                "source_url": yc_origin["canonical_url"],
            }],
            "thesis_fit_reasons": ["Automates investment-team workflows."],
        }

        candidates, excluded = self.sources.normalize_candidates([product_hunt, yc])

        self.assertEqual(excluded, [])
        self.assertEqual(len(candidates), 1)
        self.assertEqual(candidates[0]["website"], "https://fallback.example/launch")
        self.assertCountEqual(
            candidates[0]["origins"],
            [product_hunt_origin, yc_origin],
        )
    ...
    def test_hacker_news_falls_back_to_normalized_company_name(self):
        ...
        items = json.loads((FIXTURES / "hacker-news-items.json").read_text(encoding="utf-8"))
        origin_sources_before = {
            origin["source"] for origin in candidate["origins"]
        }
        self.assertEqual(origin_sources_before, {"yc"})
        ...
        origin_sources_after = {
            origin["source"] for origin in enriched[0]["origins"]
        }
        self.assertEqual(origin_sources_after, {"yc"})
        self.assertEqual(origin_sources_after, origin_sources_before)
